...
final class WareLoaderTest extends TestCase {
	 ...
	 * @param list<string>         $extra_files   Extra filenames to add.
	 */
	private function make_wp_archive( array $manifest = [], array $extra_files = [] ): string {
		$default  = [ 'name' => 'Test', 'slug' => 'test-ware', 'version' => '1.0.0', 'entry' => 'index.html' ];
	...
	public function test_rejects_php_file_inside_archive(): void {
		...
		$path   = $this->make_wp_archive( [], [ 'shell.php' ] );
		$loader = new WareLoader( $this->make_registry() );
