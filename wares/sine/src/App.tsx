    ...
    <div className="synth">
      ...
      <div className="synth__top">
        ...
        <section className="synth__module synth__module--osc">
          <h2 className="synth__module-title">{ __( 'OSCILLATOR', 'bazaar' ) }</h2>
          ...
          <div className="synth__knob-row">
            <Knob label={ __( 'VOLUME', 'bazaar' ) } value={ params.volume } min={ 0 } max={ 1 } step={ 0.01 }
              format={ v => `${ Math.round( v * 100 ) }%` }
              onChange={ v => setParam( 'volume', v ) } />
            <Knob label={ __( 'DETUNE', 'bazaar' ) } value={ params.detune } min={ -50 } max={ 50 } step={ 1 }
        ...
        <section className="synth__module synth__module--adsr">
          <h2 className="synth__module-title">{ __( 'ENVELOPE', 'bazaar' ) }</h2>
          <div className="synth__knob-row">
            <Knob label={ __( 'ATK', 'bazaar' ) } value={ params.adsr.attack }  min={ 0.001 } max={ 2 }
              format={ v => `${ v.toFixed( 2 ) }s` } onChange={ v => setAdsr( 'attack',  v ) } />
            <Knob label={ __( 'DEC', 'bazaar' ) } value={ params.adsr.decay }   min={ 0.001 } max={ 2 }
              format={ v => `${ v.toFixed( 2 ) }s` } onChange={ v => setAdsr( 'decay',   v ) } />
            <Knob label={ __( 'SUS', 'bazaar' ) } value={ params.adsr.sustain } min={ 0 } max={ 1 }
              format={ v => `${ Math.round( v * 100 ) }%` } onChange={ v => setAdsr( 'sustain', v ) } />
            <Knob label={ __( 'REL', 'bazaar' ) } value={ params.adsr.release } min={ 0.001 } max={ 3 }
