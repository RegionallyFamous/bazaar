 ...
 */
export function useWpFetch(path) {
...
export function useCurrentUser() {
    const { data: user, loading, error } = useWpFetch('/wp/v2/users/me');
 ...
 */
export function useWpPosts(query = {}) {
    ...
    const params = new URLSearchParams(Object.entries(JSON.parse(queryRef.current))
        .filter(([, v]) => v !== undefined)
        .map(([k, v]) => [k, String(v)])).toString();
    const path = `/wp/v2/posts${params ? '?' + params : ''}`;
    const result = useWpFetch(path);
