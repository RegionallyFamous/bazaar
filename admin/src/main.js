...
async function uploadWare( file ) {
	...
	try {
		...
		showError( err?.message ?? __( 'Upload failed. Please try again.', 'bazaar' ) );
	} finally {
		resetProgress();
...
gallery.addEventListener( 'change', async ( e ) => {
	...
	if ( ! input.matches( '.bazaar-toggle__input' ) ) return;
	...
	try {
		...
		showError( err?.message ?? __( 'Could not update ware status.', 'bazaar' ) );
	} finally {
...
function updateWareCount( delta ) {
	...
	if ( emptyState ) emptyState.hidden = next > 0;
}
